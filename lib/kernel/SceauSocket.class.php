   ...
   //connects with SSL protocol if secure connection asked, HTTP protocol otherwise
    ...
    if ($socket !== false) {
      ...
      //if header sending is impossible : log
        ...
        insertLogSceau(__METHOD__ . ' : ' . __LINE__, "Envoi des donn̩es impossible sur : " . $host);
        $res = false;
      }
      //closes the connexion
      fclose($socket);
    } else { //if connection failed, log
      insertLogSceau(__METHOD__ . ' : ' . __LINE__, "Connexion socket impossible sur l'h̫te $host. Erreur " . $this->errno . " : " . $this->errstr);
      $res = false;
    }
